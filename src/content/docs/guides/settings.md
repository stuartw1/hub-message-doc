...
description: Every setting in HubMessage and what it does.
---
...
|---|---|---|
| Launch at login | Off | Starts HubMessage automatically when you sign in to your Mac. Uses `SMAppService`. |
...
| Backfill window | Off | If enabled, on first sign-in HubMessage will also sync messages older than the sign-in date, going back the chosen number of days. |
...
| Full Reset & Sign Out | **Destructive.** Deletes all HubSpot communication records HubMessage created, removes the custom inbox channel, clears the local cache, deletes Keychain credentials. Leaves no trace. See [Resetting & signing out](/guides/reset/). |
| Check for updates now | Manually trigger a Sparkle update check. |

## Where settings are stored

- **Configuration**: `~/Library/Preferences/uk.nvpartners.hubmessage.plist`
- **Local message cache**: `~/Library/Containers/uk.nvpartners.hubmessage/Data/Library/Application Support/`
- **Credentials**: macOS Keychain, service name `uk.nvpartners.hubmessage`
- **Debug logs**: `~/Library/Application Support/HubMessage/Logs/` (one file per launch; older files rotate automatically)
