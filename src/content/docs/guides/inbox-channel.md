...
description: How SwitchMessage's custom iMessage channel works inside HubSpot's conversations inbox.
---

In addition to logging every message as a communication record on a contact,
SwitchMessage can stream messages into a dedicated **iMessage channel** in your
...
1. Open SwitchMessage Settings → **Inbox**.
2. Click **Set up iMessage channel**.
3. SwitchMessage will:
...
- You must have authorized SwitchMessage with the
...
SwitchMessage streams messages **in** (read-only into HubSpot); replies
...
developer account), SwitchMessage routes those specific requests through
our Cloudflare Worker at `api.switchmessage.com`, which injects the
