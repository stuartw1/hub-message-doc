...
description: Sign in to HubMessage with OAuth, or use a HubSpot Private App token.
---

HubMessage needs access to your HubSpot portal to:
...
You can authorize HubMessage with either **OAuth** (recommended) or a
...
4. The browser will redirect to a local URL on your Mac
   (`http://localhost:8491/oauth/callback`). HubMessage handles the
   redirect automatically and the popover updates to show you're
   signed in.

OAuth gives HubMessage a refresh token, so it can keep syncing in the
...
custom URL schemes (`hubmessage://`) for redirects.
:::

### Scopes HubMessage requests
...
HubMessage **does not** request marketing, file, deal, ticket, or any
...
your Private App, HubMessage will still log communications under
...
`uk.nvpartners.hubmessage`. They are never written to disk in plain
...
Once you're connected, HubMessage will perform an initial contact
