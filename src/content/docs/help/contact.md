...
description: How to get help, file a bug, or request a feature.
---
...
slower around weekends and UK public holidays.
...
The debug log is **PII-redacted before it leaves your Mac**:

- Phone numbers are truncated (only the last 3 digits remain).
- Email addresses are reduced to their domain.
- Message bodies are not logged.
- OAuth tokens and API keys are never logged.

If you'd rather check the log before sending, the same Settings
panel has a **Show debug log** option that displays it in-app.
...
We keep a backlog of feature requests prioritized by how often
they come up. Categories that are currently top of the list:

- Two-way reply (sending iMessages from HubSpot back to the
  customer).
- Attachment sync.
- Group chat support.
...
For refund requests and billing questions, see
[Subscriptions](/billing/subscriptions/). The fastest path is
to email support with your Paddle receipt number.
